...
它不要求你一开始就知道所有定义和方法。它的目标是让 Codex：
...
在新项目第一条线程中复制 `prompts/first_thread_prompt.md`，把 `<project_path>`、`<project_name>` 和你的自然语言项目目标替换掉。
...
| `AGENTS.template.md` | 新项目 Codex 工作规则模板 |
...
| `docs/codex/analysis_contract_template.md` | 分析任务合约模板 |
...
| `.gitignore.template` | 科研/MD 项目 Git 忽略模板 |
| `prompts/*.md` | 常用第一线程、设计、实现、解释、维护 prompt |
| `examples/critical_first_thread_prompt.md` | `critical` 项目第一线程示例 |
...
5. 可以大量写“未确定”，这样 Codex 会知道必须先问你。
6. 合约主要由 Codex 草拟，你只负责确认高风险定义。
7. 每个正式分析都要有可复现入口、产物说明和轻量验证。
8. 每周或每个阶段结束时，让 Codex 更新项目地图、定义文件、文献综述和方法注册表。
